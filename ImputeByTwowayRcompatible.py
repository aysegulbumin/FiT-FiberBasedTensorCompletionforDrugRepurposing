...
#STEP 1: Convert from .nc file to xarray
tensor = xr.load_dataset('/blue/tkahveci/aysegul.bumin/Data/dataset.nc').to_array()

genes = tensor.coords["G"].values
cell_lines = tensor.coords["CL"].values
drugs = tensor.coords["D"].values

#STEP 2: Convert the xarray to dictionary 
Dict = {}
for cell_line in cell_lines:
  ...
  for drug in drugs:
    GeneList= []
    for gene in genes:
      GeneList.append(tensor.loc[dict(CL=cell_line, D=drug,G=gene)].values[0])
    DrugList.append(np.asarray(GeneList))
  Dict[cell_line] = DrugList

#STEP 3: Convert the dictionary to another dictionary named Set
number_of_nans=0
counter=0
Set={}
...
for i in range(level):
    for j, gene_vector in enumerate(GeneList):
        ...
        if(len(an_array)>1):
            ...
            newGeneList.append(gene_vector)
            newSetList.append(SetList[j])
...
for each_set in SetList:
    ...
    tf_sgd_constant = TF_SGD(each_set,rank, alpha_, 120,2, train_list, test_list)
...
#UPDATE JUST THE TEST INDICES WITH TWO WAY ALGORITHM
#This has a down side, if the test is not done in run1 then the tensor completion and the two way algorithm are expected to perform exactly the same
...
for i,j in test_list:
    for k in range(978):
      ...
      ImputedTensorSeed[(i,j)][k] = predicted
# define dictionary
# Set

# create a binary pickle file 
f = open("TwoWayImputedTensorSeed"+str(cluster_seed)+"Level"+str(cluster_level)+".pkl","wb")
...
#Write the imputed tensor back to the xarray tensor
...
for key,value  in Set.items():
  ...
  d = key[1]
  for i in range(978):
    tensor.loc[dict(CL=cell_lines[c], D=drugs[d],G=genes[i])].values[0] = ImputedTensorSeed[key][i]

#Save the imputed tensor into an .nc file

tensor.to_netcdf("TwoWayImputedTensorSeed"+str(cluster_seed)+"Level"+str(cluster_level)+".nc")
