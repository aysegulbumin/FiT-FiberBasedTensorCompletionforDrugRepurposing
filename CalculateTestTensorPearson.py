...
# Obtain weighted rank correlation between x and y.

# specify weights
def weights(x, n, e=0.01):
    w = 2.0*norm.pdf(x-0.5, 0, n*e)
    return w
...
def weighted_spearman(x, y):
    ...
    for i in range(0,n):
        # get the weights
        w = weights(x_ranked[i], n)
        #w = 1/n #for uniform weights (Shieh et al. 2000)
        d = w * (x_ranked[i] - ((n+1)/2)) * (y_ranked[i] - ((n+1)/2))
        s = s + d
    
    return s
    
# weighted rank correlation
def weighted_rank_cor(x, y, positive=True):
    if(positive):
        x = np.negative(x)
        y = np.negative(y)
    
    return weighted_spearman(x, y)/weighted_spearman(x, x)
...
#cor_weighted = []
#correlations_stat = []
for key, value in CellDictPredicted.items():
    predicted = value
    actual =  CellDictActual[key]
    coeff, p_val = stats.pearsonr(predicted, actual)
    #spearman,_ = stats.spearmanr(predicted,actual)
    print("Cell Line: ",key)
    print("Pearson coeff : ",coeff)
    print("P-val : ", p_val)
