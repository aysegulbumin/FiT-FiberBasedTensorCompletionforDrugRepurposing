...
class TF_SGD(): #stands for tensor factorization
    ...
    def __init__(self,Dict, K, alpha, iterations, mode,training_sample, validation_sample, test_sample):
        """
    ...
    def train(self,mood, seed):
        ...
        training_process = []
        val_error_list = []
        val_norm_square_list =[]
        time_array=[]
        inter=0
        time_array.append(inter)
        start_=time.process_time()
        initial_mse, e1, n1 = self.mse_validation()
        ...
        for i in range(1,self.iterations):
            ...
            if i%1== 0:
                mse, val_error , val_norm_square = self.mse_validation()
                inter = time.process_time()-start_
                time_array.append(inter)
                val_error_list.append(val_error)
                val_norm_square_list.append(val_norm_square)
                training_process.append((i, mse))
                print("Iteration: %d ; error = %.4f" % (i, mse))
                print("Time:", inter)
        return training_process, time_array , val_error_list, val_norm_square_list
    ...
    def mse_test(self):
        ...
        for x, y in self.test_samples :
            ...
            self.Error[(x,y)]= np.sqrt(error.numpy())/ np.sqrt(norm_square.numpy())
            index=index+1
        return np.sqrt(summed_error.numpy())/ np.sqrt(summed_norm_square.numpy()), error.numpy() , summed_norm_square.numpy()
...
for gene_index in range(978):
    ...
    for key, values in Set.items():
        if (key not in test_list) and (key not in val_list):
...
for i in range(level):
    for j, gene_vector in enumerate(GeneList):
        ...
        newGeneList.append(Genes0)
        newGeneList.append(Genes1)
        set_ =SetList[j]
        #Sets
        Set1 = {}
        Set2 = {}
        for key,values in set_.items():
            ...
            for i2, value in enumerate(values):
                if (kmeans.labels_[i2]==0):
                    Set1[key].append(value)
                elif (kmeans.labels_[i2]==1):
                    Set2[key].append(value)
                else:
                    print("Something is wrong.")
        for key in Set1.keys():
            Set1[key]= torch.FloatTensor(Set1[key])
        for key in Set2.keys():
            Set2[key]= torch.FloatTensor(Set2[key])
        newSetList.append(Set1)
        newSetList.append(Set2)
    print("New Gene List has " +str(len(newGeneList))+" clusters.")
    print("New Set List has " +str(len(newSetList))+" clusters.")
...
len_=0
for i, each in enumerate(GeneList):
    len_ += len(each)
...
for each_set in SetList:
    ...
    tf_sgd_constant = TF_SGD(each_set,rank, alpha_, 120,2, train_list, val_list, test_list)
    general_sgd_constant, time_sgd_constant, val_error, val_norm = tf_sgd_constant.train(1,0) #, [(0, seed) for seed in range(2)]) #0 mood SGD, 1 mood AdaGrad, 2 mood ADAM
    ...
    #GenePr
    General_val_error.append(val_error)
    General_val_norm_square.append(val_norm)
...
norm = 0
for i in range(8):
    error += General_val_error[i][-1]
    norm +=General_val_norm_square[i][-1]
print(math.sqrt(error)/math.sqrt(norm))

print("Validation Error Min :")

error = 0
norm = 0
c = 0
for i in range(8):
    min_value = min(General_val_error[i])
    min_index = General_val_error[i]. index(min_value) 
    error += General_val_error[i][min_index]
    norm += General_val_norm_square[i][min_index]
print(math.sqrt(error)/math.sqrt(norm))

print("Calculating the Imputed Tensor : ")


ImputedTensorValSeed = {}

for key, value in Set.items():
    
    ImputedTensorValSeed[key] = torch.zeros(978)
    ...
    for i, each in enumerate(GeneList):
        ...
        for j, index in enumerate(gene_indices):
            ...
            ImputedTensorValSeed[key][index] = ImputedTensors[i][key][j]

# define dictionary
# Set

# create a binary pickle file 
f = open("ImputedTensorVal"+str(validation)+"Seed"+str(cluster_seed)+"Level"+str(cluster_level)+".pkl","wb")

# write the python object (dict) to pickle file
pickle.dump(ImputedTensorValSeed,f)
...
for key in test_list:
    for k in range(978):
        predicted = ImputedTensorValSeed[key][k]
        actual = Set[key][k]
