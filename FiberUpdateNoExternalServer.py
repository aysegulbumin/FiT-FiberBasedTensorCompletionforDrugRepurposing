    ...
    def sppa(self,iterations):
        ...
        for i, j, k, r in mini_samples:
            ...
            for m in range(1):
                ...
                denominator= 1+ self.l*(np.dot(bc,bc.T)+np.dot(ac,ac.T)+np.dot(ab,ab.T))
...
class TF_SGD(): #stands for tensor factorization
    ...
    def mse(self):
        ...
        for x, y in self.test_samples :
            ...
            sub=torch.sub(Fiber_actual, np.dot(torch.tensor(self.C),Hadamart_ai_bj), alpha=1)
    ...
    def sgd(self,iterations):
        ...
        for i, j in mini_samples:
            for each in range(1):
                ...
                predicted = np.dot( torch.tensor(self.C), Hadamart_ai_bj)
                sub = torch.sub(Fiber_actual, predicted, alpha=1)
                
                Grad_A = - torch.mul(np.dot(torch.tensor(self.C).T,sub),torch.tensor(self.B[j, :][:]))
                #Grad_A2 = - np.dot(torch.tensor(self.C).T ,torch.mul(sub,torch.tensor(self.B[j, :][:])))
                Grad_B = - torch.mul(np.dot(torch.tensor(self.C).T,sub),torch.tensor(self.A[i, :][:]))
                #Grad_B2 = - np.dot(torch.tensor(self.C).T, torch.mul(sub,torch.tensor(self.A[i, :][:])))
                
                transpose= Hadamart_ai_bj.T
                #print(transpose.unsqueeze(0).shape)
                #print(sub.unsqueeze(1).shape)
                #print(torch.tensor(self.C).shape)
                Grad_C = - np.dot(sub.unsqueeze(1),transpose.unsqueeze(0))
    ...
    def Adam(self,iterations):
        ...
        for i, j, k, r in mini_samples:
            ...
            v1_a= beta_2*(v1_a) +(1- beta_2)*(np.dot(gradient_a,gradient_a.T))
            v1_b= beta_2*(v1_b) +(1- beta_2)*(np.dot(gradient_b,gradient_b.T))
            v1_c= beta_2*(v1_c) +(1- beta_2)*(np.dot(gradient_c,gradient_c.T))
